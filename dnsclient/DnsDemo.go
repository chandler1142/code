...
func Send(dnsServer string, domain string) ([]byte, int, time.Duration) {
...
func Receive(recvBuf []byte) {
	r := bytes.NewReader(recvBuf)
	requestHeader := &dnsHeader{}

	binary.Read(r, binary.BigEndian, requestHeader)
	fmt.Printf("%x", requestHeader)
